...
class SalesDetailsPage extends GetView<SalesController> {
  const SalesDetailsPage({super.key});

  @override
  Widget build(BuildContext context) {
    String id = Get.parameters['id'] ?? '';
    controller.getSales(int.parse(id), false);
            ...
            children: [
              ResponsiveGridCol(
                ...
                child: _buildDetailCard("Sale Info", [
                  ...
                  _buildDetailRow(
                      "Status", sale.status == 0 ? "Received" : "Pending"),
          ...
          Row(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                      "Payment Status: ${sale.paymentStatus == 0 ? "Paid" : "Unpaid"}",
                      style: const TextStyle(
                          fontSize: 16, fontWeight: FontWeight.w400)),
                  const SizedBox(height: 10),
                  Text("Payment Type: ${sale.paymentType?.name ?? ""}",
                      style: const TextStyle(
                          fontSize: 16, fontWeight: FontWeight.w400)),
                  const SizedBox(height: 10),
                  Text("Note: ${sale.note ?? ""}",
  ...
  Widget _buildSaleItemsTable(List<SaleItem> items) {
    controller.tableHeight =
        ((controller.salesResponse.value!.saleItems.length + 1) * 50);
    return SizedBox(
